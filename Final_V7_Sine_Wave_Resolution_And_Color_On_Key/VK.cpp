...
//Sine Wave related variables for 1024
float pos_1024_graphics[1024] [1024] [4];
VertexData vertexData_position_1024_x_1024_graphics;
VkCommandBuffer *vkCommandBuffer_for_1024_x_1024_graphics_array = NULL;
BOOL bMesh1024_chosen = true;

//Sine Wave related variables for 512
float pos_512_graphics[512] [512] [4];
VertexData vertexData_position_512_x_512_graphics;
VkCommandBuffer *vkCommandBuffer_for_512_x_512_graphics_array = NULL;
BOOL bMesh512_chosen = false;

//Sine Wave related variables for 256
float pos_256_graphics[256] [256] [4];
VertexData vertexData_position_256_x_256_graphics;
VkCommandBuffer *vkCommandBuffer_for_256_x_256_graphics_array = NULL;
BOOL bMesh256_chosen = false;

//Sine Wave related variables for 128
float pos_128_graphics[128] [128] [4];
VertexData vertexData_position_128_x_128_graphics;
VkCommandBuffer *vkCommandBuffer_for_128_x_128_graphics_array = NULL;
BOOL bMesh128_chosen = false;

//Sine Wave related variables for 64
float pos_64_graphics[64] [64] [4];
VertexData vertexData_position_64_x_64_graphics;
VkCommandBuffer *vkCommandBuffer_for_64_x_64_graphics_array = NULL;
BOOL bMesh64_chosen = false;
	...
	switch (iMsg)
	{
		...
		*/
		...
		case WM_CHAR:
			...
			case '1':
				bMesh1024_chosen = true;
				bMesh512_chosen = false;
				bMesh256_chosen = false;
				bMesh128_chosen = false;
				bMesh64_chosen = false;
				FileIO("WndProc() WM_CHAR(1 key)-> 1024 x 1024 mesh selected.\n");
				break;

			case '5':
				bMesh512_chosen = true;
				bMesh1024_chosen = false;
				bMesh256_chosen = false;
				bMesh128_chosen = false;
				bMesh64_chosen = false;
				FileIO("WndProc() WM_CHAR(5 key)-> 512 x 512 mesh selected.\n");
				break;

			case '2':
				bMesh256_chosen = true;
				bMesh1024_chosen = false;
				bMesh512_chosen = false;
				bMesh128_chosen = false;
				bMesh64_chosen = false;
				FileIO("WndProc() WM_CHAR(2 key)-> 256 x 256 mesh selected.\n");
				break;

			case '8':
				bMesh128_chosen = true;
				bMesh1024_chosen = false;
				bMesh512_chosen = false;
				bMesh256_chosen = false;
				bMesh64_chosen = false;
				FileIO("WndProc() WM_CHAR(8 key)-> 128 x 128 mesh selected.\n");
				break;
			...
			case '6':
				...
				bMesh256_chosen = false;
				bMesh128_chosen = false;
				FileIO("WndProc() WM_CHAR(6 key)-> 64 x 64 mesh selected.\n");
				break;
...
VkResult initialize(void)
{
	...
	*/
	...
	//Initialize Sine wave array for 1024 x 1024
	initializeSineWaveArrays(1024, 1024);
	
	/*
	22.2. Declare User defined function CreateVertexBuffer() for 1024 x 1024.
	Write its prototype below CreateCommandBuffers() and above CreateRenderPass() and also call it between the calls of these two.
	*/
	memset((void*)&vertexData_position_1024_x_1024_graphics, 0, sizeof(VertexData));
	vkResult  = CreateVertexBuffer(1024, 1024, &vertexData_position_1024_x_1024_graphics);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateVertexBuffer() function failed with error code for 1024 x 1024 %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateVertexBuffer() succedded for 1024 x 1024\n");
	}

	vkResult  = CreateCommandBuffers(&vkCommandBuffer_for_512_x_512_graphics_array);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateCommandBuffers() function failed with error code %d for 512 x 512 command buffer\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateCommandBuffers() succedded  for 512 x 512 command buffer\n");
	}

	//Initialize Sine wave array for 512 x 512
	initializeSineWaveArrays(512, 512);

	/*
	22.2. Declare User defined function CreateVertexBuffer() for 512 x 512.
	Write its prototype below CreateCommandBuffers() and above CreateRenderPass() and also call it between the calls of these two.
	*/
	memset((void*)&vertexData_position_512_x_512_graphics, 0, sizeof(VertexData));
	vkResult  = CreateVertexBuffer(512, 512, &vertexData_position_512_x_512_graphics);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateVertexBuffer() function failed with error code for 512 x 512 %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateVertexBuffer() succedded for 512 x 512\n");
	}

	//Create CommandBuffers for 256 x 256
	vkResult  = CreateCommandBuffers(&vkCommandBuffer_for_256_x_256_graphics_array);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateCommandBuffers() function failed with error code %d for 256 x 256 command buffer\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateCommandBuffers() succedded  for 256 x 256 command buffer\n");
	}

	//Initialize Sine wave array for 256 x 256
	initializeSineWaveArrays(256, 256);

	//CreateVertexBuffer for 256 x 256
	memset((void*)&vertexData_position_256_x_256_graphics, 0, sizeof(VertexData));
	vkResult  = CreateVertexBuffer(256, 256, &vertexData_position_256_x_256_graphics);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateVertexBuffer() function failed with error code for 256 x 256 %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateVertexBuffer() succedded for 256 x 256\n");
	}

	//Create CommandBuffers for 128 x 128
	vkResult  = CreateCommandBuffers(&vkCommandBuffer_for_128_x_128_graphics_array);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateCommandBuffers() function failed with error code %d for 128 x 128 command buffer\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateCommandBuffers() succedded  for 128 x 128 command buffer\n");
	}

	//Initialize Sine wave array for 128 x 128
	initializeSineWaveArrays(128, 128);

	//CreateVertexBuffer for 128 x 128
	memset((void*)&vertexData_position_128_x_128_graphics, 0, sizeof(VertexData));
	vkResult  = CreateVertexBuffer(128, 128, &vertexData_position_128_x_128_graphics);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateVertexBuffer() function failed with error code for 128 x 128 %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateVertexBuffer() succedded for 128 x 128\n");
	}

	//Create CommandBuffers for 64 x 64
	vkResult  = CreateCommandBuffers(&vkCommandBuffer_for_64_x_64_graphics_array);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateCommandBuffers() function failed with error code %d for 64 x 64 command buffer\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateCommandBuffers() succedded  for 64 x 64 command buffer\n");
	}

	//Initialize Sine wave array for 64 x 64
	initializeSineWaveArrays(64, 64);

	//CreateVertexBuffer for 64 x 64
	memset((void*)&vertexData_position_64_x_64_graphics, 0, sizeof(VertexData));
	vkResult  = CreateVertexBuffer(64, 64, &vertexData_position_64_x_64_graphics);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("initialize(): CreateVertexBuffer() function failed with error code for 64 x 64 %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("initialize(): CreateVertexBuffer() succedded for 64 x 64\n");
	}
...
VkResult resize(int width, int height)
{
	...
	*/
	...
	//Destroy Commandbuffer for 512 x 512
	for(uint32_t i =0; i < swapchainImageCount; i++)
	{
		vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_512_x_512_graphics_array[i]);
		FileIO("resize(): vkFreeCommandBuffers() is done for 512 x 512\n");
	}

	//Free actual command buffer array for 512 x 512
	if(vkCommandBuffer_for_512_x_512_graphics_array)
	{
		free(vkCommandBuffer_for_512_x_512_graphics_array);
		vkCommandBuffer_for_512_x_512_graphics_array = NULL;
		FileIO("resize(): vkCommandBuffer_for_512_x_512_graphics_array is freed\n");
	}

	//Destroy Commandbuffer for 256 x 256
	for(uint32_t i =0; i < swapchainImageCount; i++)
	{
		vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_256_x_256_graphics_array[i]);
		FileIO("resize(): vkFreeCommandBuffers() is done for 256 x 256\n");
	}

	//Free actual command buffer array for 256 x 256
	if(vkCommandBuffer_for_256_x_256_graphics_array)
	{
		free(vkCommandBuffer_for_256_x_256_graphics_array);
		vkCommandBuffer_for_256_x_256_graphics_array = NULL;
		FileIO("resize(): vkCommandBuffer_for_256_x_256_graphics_array is freed\n");
	}

	//Destroy Commandbuffer for 128 x 128
	for(uint32_t i =0; i < swapchainImageCount; i++)
	{
		vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_128_x_128_graphics_array[i]);
		FileIO("resize(): vkFreeCommandBuffers() is done for 128 x 128\n");
	}

	//Free actual command buffer array for 128 x 128
	if(vkCommandBuffer_for_128_x_128_graphics_array)
	{
		free(vkCommandBuffer_for_128_x_128_graphics_array);
		vkCommandBuffer_for_128_x_128_graphics_array = NULL;
		FileIO("resize(): vkCommandBuffer_for_128_x_128_graphics_array is freed\n");
	}

	//Destroy Commandbuffer for 64 x 64
	for(uint32_t i =0; i < swapchainImageCount; i++)
	{
		vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_64_x_64_graphics_array[i]);
		FileIO("resize(): vkFreeCommandBuffers() is done for 64 x 64\n");
	}

	//Free actual command buffer array for 64 x 64
	if(vkCommandBuffer_for_64_x_64_graphics_array)
	{
		free(vkCommandBuffer_for_64_x_64_graphics_array);
		vkCommandBuffer_for_64_x_64_graphics_array = NULL;
		FileIO("resize(): vkCommandBuffer_for_64_x_64_graphics_array is freed\n");
	}
	...
	*/
	...
	//30.16 Create CommandBuffers for 512 x 512
	vkResult  = CreateCommandBuffers(&vkCommandBuffer_for_512_x_512_graphics_array);
	if (vkResult != VK_SUCCESS)
	{
		FileIO("resize(): CreateCommandBuffers() function failed with error code %d for 512 x 512\n", vkResult);
		return vkResult;
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		FileIO("resize(): CreateCommandBuffers() function failed with error code %d for 256 x 256\n", vkResult);
		return vkResult;
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		FileIO("resize(): CreateCommandBuffers() function failed with error code %d for 128 x 128\n", vkResult);
		return vkResult;
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		FileIO("resize(): CreateCommandBuffers() function failed with error code %d for 64 x 64\n", vkResult);
		return vkResult;
	}
...
VkResult display(void)
{
	...
	*/
	...
	else if(bMesh512_chosen == TRUE)
	{
		vkCommandBuffer_array = vkCommandBuffer_for_512_x_512_graphics_array;
	}
	else if(bMesh256_chosen == TRUE)
	{
		vkCommandBuffer_array = vkCommandBuffer_for_256_x_256_graphics_array;
	}
	else if(bMesh128_chosen == TRUE)
	{
		vkCommandBuffer_array = vkCommandBuffer_for_128_x_128_graphics_array;
	}
	else if(bMesh64_chosen == TRUE)
	{
		vkCommandBuffer_array = vkCommandBuffer_for_64_x_64_graphics_array;
	}
		...
		if(vkDevice)
		{
			...
			*/
			...
			void vkDestroyBuffer(
				...
				const VkAllocationCallbacks*                pAllocator);
			*/
			if(vertexData_position_1024_x_1024_graphics.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, vertexData_position_1024_x_1024_graphics.vkDeviceMemory, NULL);
				vertexData_position_1024_x_1024_graphics.vkDeviceMemory = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_1024_x_1024_graphics.vkDeviceMemory is freed\n");
			}
			
			if(vertexData_position_1024_x_1024_graphics.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, vertexData_position_1024_x_1024_graphics.vkBuffer, NULL);
				vertexData_position_1024_x_1024_graphics.vkBuffer = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_1024_x_1024_graphics.vkBuffer is freed\n");
			}
			...
			//Cleanup for 512 x 512 vertex data
			if(vertexData_position_512_x_512_graphics.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, vertexData_position_512_x_512_graphics.vkDeviceMemory, NULL);
				vertexData_position_512_x_512_graphics.vkDeviceMemory = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_512_x_512_graphics.vkDeviceMemory is freed\n");
			}

			if(vertexData_position_512_x_512_graphics.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, vertexData_position_512_x_512_graphics.vkBuffer, NULL);
				vertexData_position_512_x_512_graphics.vkBuffer = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_512_x_512_graphics.vkBuffer is freed\n");
			}

			//Free command buffers for 512 x 512
			for(uint32_t i =0; i < swapchainImageCount; i++)
			{
				vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_512_x_512_graphics_array[i]);
				FileIO("uninitialize(): vkFreeCommandBuffers() is done for 512 x 512\n");
			}
			...
			if(vkCommandBuffer_for_512_x_512_graphics_array)
			{
				free(vkCommandBuffer_for_512_x_512_graphics_array);
				vkCommandBuffer_for_512_x_512_graphics_array = NULL;
				FileIO("uninitialize(): vkCommandBuffer_for_512_x_512_graphics_array is freed\n");
			}

			//Cleanup for 256 x 256 vertex data
			if(vertexData_position_256_x_256_graphics.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, vertexData_position_256_x_256_graphics.vkDeviceMemory, NULL);
				vertexData_position_256_x_256_graphics.vkDeviceMemory = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_256_x_256_graphics.vkDeviceMemory is freed\n");
			}

			if(vertexData_position_256_x_256_graphics.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, vertexData_position_256_x_256_graphics.vkBuffer, NULL);
				vertexData_position_256_x_256_graphics.vkBuffer = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_256_x_256_graphics.vkBuffer is freed\n");
			}

			//Free command buffers for 256 x 256
			for(uint32_t i =0; i < swapchainImageCount; i++)
			{
				vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_256_x_256_graphics_array[i]);
				FileIO("uninitialize(): vkFreeCommandBuffers() is done for 256 x 256\n");
			}
			...
			if(vkCommandBuffer_for_256_x_256_graphics_array)
			{
				free(vkCommandBuffer_for_256_x_256_graphics_array);
				vkCommandBuffer_for_256_x_256_graphics_array = NULL;
				FileIO("uninitialize(): vkCommandBuffer_for_256_x_256_graphics_array is freed\n");
			}

			//Cleanup for 128 x 128 vertex data
			if(vertexData_position_128_x_128_graphics.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, vertexData_position_128_x_128_graphics.vkDeviceMemory, NULL);
				vertexData_position_128_x_128_graphics.vkDeviceMemory = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_128_x_128_graphics.vkDeviceMemory is freed\n");
			}

			if(vertexData_position_128_x_128_graphics.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, vertexData_position_128_x_128_graphics.vkBuffer, NULL);
				vertexData_position_128_x_128_graphics.vkBuffer = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_128_x_128_graphics.vkBuffer is freed\n");
			}

			//Free command buffers for 128 x 128
			for(uint32_t i =0; i < swapchainImageCount; i++)
			{
				vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_128_x_128_graphics_array[i]);
				FileIO("uninitialize(): vkFreeCommandBuffers() is done for 128 x 128\n");
			}
			...
			if(vkCommandBuffer_for_128_x_128_graphics_array)
			{
				free(vkCommandBuffer_for_128_x_128_graphics_array);
				vkCommandBuffer_for_128_x_128_graphics_array = NULL;
				FileIO("uninitialize(): vkCommandBuffer_for_128_x_128_graphics_array is freed\n");
			}

			//Cleanup for 64 x 64 vertex data
			if(vertexData_position_64_x_64_graphics.vkDeviceMemory)
			{
				vkFreeMemory(vkDevice, vertexData_position_64_x_64_graphics.vkDeviceMemory, NULL);
				vertexData_position_64_x_64_graphics.vkDeviceMemory = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_64_x_64_graphics.vkDeviceMemory is freed\n");
			}

			if(vertexData_position_64_x_64_graphics.vkBuffer)
			{
				vkDestroyBuffer(vkDevice, vertexData_position_64_x_64_graphics.vkBuffer, NULL);
				vertexData_position_64_x_64_graphics.vkBuffer = VK_NULL_HANDLE;
				FileIO("uninitialize(): vertexData_position_64_x_64_graphics.vkBuffer is freed\n");
			}

			//Free command buffers for 64 x 64
			for(uint32_t i =0; i < swapchainImageCount; i++)
			{
				vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_for_64_x_64_graphics_array[i]);
				FileIO("uninitialize(): vkFreeCommandBuffers() is done for 64 x 64\n");
			}
			...
			if(vkCommandBuffer_for_64_x_64_graphics_array)
			{
				free(vkCommandBuffer_for_64_x_64_graphics_array);
				vkCommandBuffer_for_64_x_64_graphics_array = NULL;
				FileIO("uninitialize(): vkCommandBuffer_for_64_x_64_graphics_array is freed\n");
			}
...
2. Declare User defined function CreateVertexBuffer().
   Write its prototype below CreateCommandBuffers() and above CreateRenderPass() and also call it between the calls of these two.
*/
VkResult CreateVertexBuffer(unsigned int mesh_width, unsigned int mesh_height, VertexData *pVertexData)
{
	//Variable declarations	
	VkResult vkResult = VK_SUCCESS;
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		FileIO("CreateVertexBuffer(): vkCreateBuffer() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("CreateVertexBuffer(): vkCreateBuffer() succedded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		FileIO("CreateVertexBuffer(): vkAllocateMemory() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("CreateVertexBuffer(): vkAllocateMemory() succedded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		FileIO("CreateVertexBuffer(): vkBindBufferMemory() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("CreateVertexBuffer(): vkBindBufferMemory() succedded\n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		FileIO("CreateVertexBuffer(): vkMapMemory() function failed with error code %d\n", vkResult);
		return vkResult;
	}
	else
	{
		FileIO("CreateVertexBuffer(): vkMapMemory() succedded\n");
	}
	...
	*/
	for(uint32_t i =0; i< swapchainImageCount; i++)
	{
		...
		*/
		...
		if(bMesh1024_chosen == TRUE)
		{
			vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_position_1024_x_1024_graphics.vkBuffer, vkDeviceSize_offset_array); //Here recording
		}
		else if(bMesh512_chosen == TRUE)
		{
			vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_position_512_x_512_graphics.vkBuffer, vkDeviceSize_offset_array); //Here recording
		}
		else if(bMesh256_chosen == TRUE)
		{
			vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_position_256_x_256_graphics.vkBuffer, vkDeviceSize_offset_array); //Here recording
		}
		else if(bMesh128_chosen == TRUE)
		{
			vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_position_128_x_128_graphics.vkBuffer, vkDeviceSize_offset_array); //Here recording
		}
		else if(bMesh64_chosen == TRUE)
		{
			vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_position_64_x_64_graphics.vkBuffer, vkDeviceSize_offset_array); //Here recording
		}
...
VkResult prepareSineWaveforCPU(unsigned int mesh_width, unsigned int mesh_height, float anim_Time)
{
	...
	*/
	...
	if( (mesh_width == 1024) && (mesh_height == 1024))
	{
		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkMapMemory.html
		vkResult = vkMapMemory(vkDevice, vertexData_position_1024_x_1024_graphics.vkDeviceMemory, 0, size, 0, &data);
		if(vkResult != VK_SUCCESS)
		{
			FileIO("prepareSineWaveforCPU():vkMapMemory() failed\n");
		}

		memcpy(data, pos_1024_graphics, size);

		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkUnmapMemory.html
		vkUnmapMemory(vkDevice, vertexData_position_1024_x_1024_graphics.vkDeviceMemory);
	}
	else if( (mesh_width == 512) && (mesh_height == 512))
	{
		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkMapMemory.html
		vkResult = vkMapMemory(vkDevice, vertexData_position_512_x_512_graphics.vkDeviceMemory, 0, size, 0, &data);
		if(vkResult != VK_SUCCESS)
		{
			FileIO("prepareSineWaveforCPU():vkMapMemory() failed for 512 x 512\n");
		}

		memcpy(data, pos_512_graphics, size);

		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkUnmapMemory.html
		vkUnmapMemory(vkDevice, vertexData_position_512_x_512_graphics.vkDeviceMemory);
	}
	else if( (mesh_width == 256) && (mesh_height == 256))
	{
		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkMapMemory.html
		vkResult = vkMapMemory(vkDevice, vertexData_position_256_x_256_graphics.vkDeviceMemory, 0, size, 0, &data);
		if(vkResult != VK_SUCCESS)
		{
			FileIO("prepareSineWaveforCPU():vkMapMemory() failed for 256 x 256\n");
		}

		memcpy(data, pos_256_graphics, size);

		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkUnmapMemory.html
		vkUnmapMemory(vkDevice, vertexData_position_256_x_256_graphics.vkDeviceMemory);
	}
	else if( (mesh_width == 128) && (mesh_height == 128))
	{
		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkMapMemory.html
		vkResult = vkMapMemory(vkDevice, vertexData_position_128_x_128_graphics.vkDeviceMemory, 0, size, 0, &data);
		if(vkResult != VK_SUCCESS)
		{
			FileIO("prepareSineWaveforCPU():vkMapMemory() failed for 128 x 128\n");
		}

		memcpy(data, pos_128_graphics, size);

		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkUnmapMemory.html
		vkUnmapMemory(vkDevice, vertexData_position_128_x_128_graphics.vkDeviceMemory);
	}
	else if( (mesh_width == 64) && (mesh_height == 64))
	{
		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkMapMemory.html
		vkResult = vkMapMemory(vkDevice, vertexData_position_64_x_64_graphics.vkDeviceMemory, 0, size, 0, &data);
		if(vkResult != VK_SUCCESS)
		{
			FileIO("prepareSineWaveforCPU():vkMapMemory() failed for 64 x 64\n");
		}

		memcpy(data, pos_64_graphics, size);

		//https://registry.khronos.org/VulkanSC/specs/1.0-extensions/man/html/vkUnmapMemory.html
		vkUnmapMemory(vkDevice, vertexData_position_64_x_64_graphics.vkDeviceMemory);
	}
	...
	for(int i =0; i < (int)mesh_width; i++)
	{
		for(int j =0; j < (int)mesh_height; j++)
		{
			for(int k =0; k < 4; k++)
			{
				...
				float w = sinf(u * frequency + anim_Time) * cosf(v * frequency + anim_Time) * 0.5f;
				if(k == 0)
				{
					if( (mesh_width == 1024) && (mesh_height == 1024))
					{
